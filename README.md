...
> 第四阶段（学校统一身份认证、多级组织管理、学校级预算熔断等）尚未实现。
...
app/
  ...
  routers/
    ...
    admin.py     管理后台（用户/Key/课题组/告警/订单/补偿，方案第十五节）
frontend/
  index.html     自包含单页前端（聊天/钱包/Token/批量/充值/贡献/文档/管理）
tests/
  test_api.py    第一阶段端到端测试
  test_phase2.py 第二阶段端到端测试（批量/队列/课题组/告警）
  test_phase3.py 第三阶段端到端测试（充值/贡献/补偿/备用池）
```
